...
from audioclas.data_utils import (
    load_data_splits,
    load_class_names,
    data_sequence,
    generate_embeddings,
...
def train_fn(TIMESTAMP, CONF):
    ...
    if not CONF["preprocessing"][
        "compute_embeddings"
    ]:  # no need to compute if embeddings are precomputed
    ...
    model_wrap = ModelWrapper(
        classifier_model=os.path.join(
            paths.get_models_dir(), "audioset", "ckpts", "final_model.h5"
        )
    )
    ...
    if CONF["preprocessing"]["compute_embeddings"]:
        ...
        X_train, y_train = generate_embeddings(
            model_wrap=model_wrap, filepaths=X_train, labels=y_train
        )
        save_embeddings_txt(X_train, y_train, "train_emb.txt")

        if CONF["training"]["use_validation"]:
            X_val, y_val = generate_embeddings(
                model_wrap=model_wrap, filepaths=X_val, labels=y_val
            )
        ...
        if not X_train[0].endswith(".npy"):
            raise Exception(
                "If you do not compute the embeddings, your train/val.txt should point to the"
